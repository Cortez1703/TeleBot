...
async def get_some_inventory(message: types.Message, state: FSMContext):
    ...
    if int(message.text) <= 0:
        if int(message.text) == 0:
            await message.answer(f"""Вы ввели 0, т.е. ничего не возращаете. Введите новое число или 
же нажмите кнопку 'Отменить возврат'.
Напомню, вы брали {user_data['inventory_name']} в количестве {user_data['inventory_amount']}""",
                                 reply_markup=button_cancel)
        else:
            await message.answer(f"""Введено некорректное число. Повторите ввод ли же 
нажмите кнопку 'Отменить возврат'
Напомню, вы брали {user_data['inventory_name']} в количестве {user_data['inventory_amount']}""",
                                 reply_markup=button_cancel)
...
У вас осталось {int(user_amount) - int(message.text)}""")
        ...
        await message.answer(f"""Вы ввели число большее, чем вы брали. Введите данные корректно
Напомню, вы брали {user_data['inventory_name']} в количестве {user_data['inventory_amount']}""",
                             reply_markup=button_cancel)
